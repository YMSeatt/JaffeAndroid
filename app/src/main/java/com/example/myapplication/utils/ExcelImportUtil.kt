...
object ExcelImportUtil {
    ...
    // Helper function to safely get string value from a cell
    ...
    ): Result<Int> = withContext(Dispatchers.IO) { // Return type changed to Result<Int>
        try {
            context.contentResolver.openInputStream(uri)?.use { inputStream ->
                val workbook = WorkbookFactory.create(inputStream)
                val sheet = workbook.getSheetAt(0) // Assuming data is in the first sheet

                if (sheet == null || sheet.physicalNumberOfRows <= 1) { // <= 1 to account for header
                ...
                for (cell in headerRow) {
                    val cellValue = getCellStringValue(cell)?.lowercase() // Use helper
                    if (cellValue != null) {
                        when {
                ...
                for (i in 1..sheet.lastRowNum) { // Start from row 1 (after header)
                ...
                Result.success(importedStudentCount) // Return count of imported students
    ...
    suspend fun exportToExcel(
        ...
        try {
            val workbook = XSSFWorkbook() // Create a new .xlsx workbook
            ...
            students.forEachIndexed { index, student ->
                val dataRow = sheet.createRow(index + 1) // Start from row 1
            ...
            Result.failure(Exception("Error exporting students: ${e.message}", e))
        }
    }
}
