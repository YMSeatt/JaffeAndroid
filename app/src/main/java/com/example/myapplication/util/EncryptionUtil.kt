...
/**
 * A utility for handling Fernet encryption and decryption.
 * This class is designed to be testable and stateless from an Android perspective.
 * It receives the encryption key via its constructor.
 *
 * @param key The 32-byte key used for encryption and signing.
 */
class EncryptionUtil(key: ByteArray) {

    private val fernetCipher: FernetCipher = FernetCipher(key)

    companion object {
        const val TTL_SECONDS = 60 * 60 // 1 hour TTL for decryption
    }
