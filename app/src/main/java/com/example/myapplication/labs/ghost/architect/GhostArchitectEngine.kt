...
object GhostArchitectEngine {
     ...
     * - **STABILITY**: Measures dispersion of "high-energy" students. Synergy peaks (1.0)
     *   when students with high negative counts are moved toward the classroom center
     *   (2000, 2000) for better oversight.
     *
    ...
    fun calculateSynergy(
        ...
        return when (goal) {
            ...
            StrategicGoal.STABILITY -> {
                ...
                // BOLT: Pre-calculate negative counts to avoid O(N*L) nested loop
                ...
                for (i in students.indices) {
                    ...
                    val negCount = negativeCounts[student.id.toLong()] ?: 0

                    if (negCount > 2) {
                        val dx = student.xPosition.value - centerX
                        val dy = student.yPosition.value - centerY
                        val dist = sqrt(dx * dx + dy * dy)
                        totalDist += min(dist.toDouble(), 2000.0)
                        count++
                    }
                }
    ...
    fun generateReport(
        ...
        val collabScore = calculateSynergy(students, edges, behaviorLogs, StrategicGoal.COLLABORATION)
        val focusScore = calculateSynergy(students, edges, behaviorLogs, StrategicGoal.FOCUS)
        val stabilityScore = calculateSynergy(students, edges, behaviorLogs, StrategicGoal.STABILITY)

        val recommendation = when {
            collabScore < 0.5f -> "Prioritize **COLLABORATION** alignment to bring social allies closer."
            focusScore < 0.5f -> "Prioritize **FOCUS** alignment to separate friction-prone student pairs."
            stabilityScore < 0.5f -> "Prioritize **STABILITY** alignment to diffuse high-energy nodes towards oversight zones."
            else -> "The current layout exhibits high neural alignment across all strategic goals."
        }
...
- **Stability Balance:** ${"%.1f".format(Locale.US, stabilityScore * 100)}%
