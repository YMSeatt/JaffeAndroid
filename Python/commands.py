...
class Command:
    """
    Abstract base class for a reversible operation.

    The Command pattern is central to the application's Undo/Redo system and persistence
    model. Each user action that modifies the application state is encapsulated as a
    Command object, capturing all necessary context to both apply and revert the change.

    ### Command Lifecycle:
    1. **Creation**: A Command is instantiated with a snapshot of the current state or
       the intended changes.
    2. **Execution**: `execute()` is called to apply the changes to the `app` state.
    3. **Undo**: `undo()` is called to revert the changes, restoring the state using
       the captured snapshots.
    4. **Serialization**: `to_dict()` converts the command into a JSON-serializable
       dictionary for persistent history storage.
    5. **Reconstruction**: `from_dict()` (and `_from_serializable_data` in subclasses)
       re-instantiates a command from its serialized dictionary form.
    """
    def __init__(self, app, timestamp=None):
        """
        Initializes the command.

        :param app: The main SeatingChartApp instance.
        :param timestamp: ISO format string of when the command was created.
        """
        self.app = app
    ...
    def execute(self):
        """
        Applies the command's action to the application state.
        Subclasses must implement this to perform the actual state modification.
        """
        raise NotImplementedError

    def undo(self):
        """
        Reverses the command's action, restoring the previous application state.
        Subclasses must implement this using the data captured during initialization.
        """
        raise NotImplementedError

    def to_dict(self):
        """
        Serializes the command to a dictionary for history persistence.

        :return: A dictionary containing the command 'type', 'timestamp', and a
                 'data' payload generated by `_get_data_for_serialization`.
        """
    ...
    def _get_data_for_serialization(self):
        """
        Returns a dictionary of internal state for serialization.
        Subclasses must implement this to define which attributes are persistent.
        """
        raise NotImplementedError
    ...
    def from_dict(cls, app, data_dict):
        """
        Reconstructs a Command object from a serialized dictionary.

        This factory method uses reflection to identify the correct Command subclass
        based on the 'type' field in the dictionary.

        :param app: The main SeatingChartApp instance.
        :param data_dict: The serialized dictionary from `to_dict()`.
        :return: An instance of the specific Command subclass, or None if the type is unknown.
        """
    ...
    def _from_serializable_data(cls, app, data, timestamp):
        """
        Subclass-specific factory method to re-instantiate from a data payload.

        :param app: The main SeatingChartApp instance.
        :param data: The 'data' payload from the serialized dictionary.
        :param timestamp: The original creation timestamp.
        :return: An instance of the Command subclass.
        """
        raise NotImplementedError
...
class MoveGuideCommand(Command):
    """
    Command to move one or more visual guides on the canvas.

    :param items_moves: A list of dictionaries, each containing:
        - 'id': The unique identifier of the guide.
        - 'old_coord': The logical world coordinate before the move.
        - 'new_coord': The logical world coordinate after the move.
    """
...
class AddGuideCommand(Command):
    """
    Command to add a new visual guide (horizontal or vertical) to the canvas.

    :param item_id: The unique identifier generated for the new guide.
    :param item_type: The type of guide ('horizontal' or 'vertical').
    :param item_data: A dictionary containing the guide's properties (e.g., 'world_coord', 'type').
    :param id_next_num: The state of the application's ID counter before adding this guide,
                        allowing for consistent ID generation during undo/redo.
    """
...
class DeleteGuideCommand(Command):
    """
    Command to remove a visual guide from the canvas.

    This command captures a full snapshot of the guide's data before deletion
    to ensure it can be perfectly restored on the canvas.

    :param item_id: The unique identifier of the guide to delete.
    :param item_data: A dictionary containing the guide's properties (captured snapshot).
    """
...
class MoveItemsCommand(Command):
    """
    Command to move multiple seating chart items (students or furniture) in bulk.

    :param items_moves: A list of dictionaries, each containing:
        - 'id': The unique identifier of the item.
        - 'type': The item type ('student' or 'furniture').
        - 'old_x', 'old_y': The logical coordinates before the move.
        - 'new_x', 'new_y': The logical coordinates after the move.
    """
    def __init__(self, app, items_moves, timestamp=None):
        super().__init__(app, timestamp)
        self.items_moves = items_moves # List of dicts: {'id', 'type', 'old_x', 'old_y', 'new_x', 'new_y'}
...
class AddItemCommand(Command):
    """
    Command to add a new student or furniture item to the seating chart.

    This command handles the addition of items and manages the application's
    internal ID counters to ensure that redoing an 'Add' operation doesn't
    cause ID collisions or sequence gaps.

    :param item_id: The unique identifier for the new item.
    :param item_type: The type of item ('student' or 'furniture').
    :param item_data: A dictionary containing the item's properties (name, position, etc.).
    :param old_next_id_num: The state of the relevant ID counter before the addition.
    """
...
class DeleteItemCommand(Command):
    """
    Command to delete a student or furniture item.

    This is a "High-Integrity" command. When a student is deleted, it doesn't
    just remove the student record; it captures a complete snapshot of all
    associated behavioral, quiz, and homework logs. This ensures that undoing
    a deletion restores the student's entire history, maintaining data continuity.

    :param item_id: The unique identifier of the item to delete.
    :param item_type: The type of item ('student' or 'furniture').
    :param item_data: A snapshot of the item's properties before deletion.
    :param associated_logs: A list of behavior/quiz log snapshots (student-only).
    """
...
class LogEntryCommand(Command):
    """
    Command to record a behavior or quiz incident for a student.

    This command adds an entry to `app.behavior_log` and ensures the log
    remains sorted chronologically by timestamp.

    :param log_entry: A dictionary containing log details (type, behavior, timestamp, etc.).
    :param student_id: The ID of the student the log belongs to.
    """
...
class LogHomeworkEntryCommand(Command):
    """
    Command to record a homework completion status or detailed mark for a student.

    Similar to `LogEntryCommand`, but targets the `app.homework_log` collection.

    :param log_entry: A dictionary containing homework log details.
    :param student_id: The ID of the student the log belongs to.
    """
    def __init__(self, app, log_entry, student_id, timestamp=None):
        super().__init__(app, timestamp)
...
class EditItemCommand(Command):
    """
    Command to modify properties of an existing student or furniture item.

    This command uses a "Snapshot-and-Change" pattern. It stores a full snapshot of
    the data before the change (`old_item_data_snapshot`) and a dictionary of
    only the modified fields (`new_item_data_changes`).

    :param item_id: The ID of the item being edited.
    :param item_type: The type of item ('student' or 'furniture').
    :param old_item_data: The complete data dictionary before modification.
    :param new_item_data_changes: A dictionary of key-value pairs representing the updates.
    """
...
class ChangeItemsSizeCommand(Command):
    """
    Command to resize one or more seating chart items in bulk.

    This command updates both the base dimensions and `style_overrides` for students.

    :param items_sizes_changes: A list of dictionaries, each containing:
        - 'id': Item identifier.
        - 'type': Item type.
        - 'old_w', 'old_h': Dimensions before resizing.
        - 'new_w', 'new_h': Dimensions after resizing.
    """
...
class MarkLiveQuizQuestionCommand(Command):
    ...
    This command tracks incremental quiz scores within the `app.live_quiz_scores`
    dictionary, maintaining a 'correct' count and a 'total_asked' count.

    :param student_id: The ID of the student being marked.
    :param action_taken: The result of the question ('correct' or 'incorrect').
    """
...
class MarkLiveHomeworkCommand(Command):
    ...
    This command supports two different tracking modes:
    1. **"Yes/No"**: Maps homework type names to specific completion statuses.
    2. **"Select"**: Stores a list of multiple selected status options for a student.

    :param student_id: The ID of the student being marked.
    :param homework_actions: The specific status update (dict for "Yes/No", list for "Select").
    :param session_mode: The interaction mode ('Yes/No' or 'Select').
    """
...
class ChangeStudentStyleCommand(Command):
    ...
    This command manages selective property updates within a student's
    `style_overrides` dictionary.

    :param student_id: The ID of the student being styled.
    :param style_property: The name of the style property (e.g., 'fill_color').
    :param old_value: The value before the change (or None if it didn't exist).
    :param new_value: The new value to apply (or None to remove the override).
    """
...
class ManageStudentGroupCommand(Command):
    """
    Command to manage student group definitions and assignments.

    This is a bulk management command that captures comprehensive snapshots of:
    - Group definitions (`student_groups`).
    - Individual student group assignments (`group_id` fields).
    - The `next_group_id_num` counter for consistent auto-generation.

    :param old_groups_snapshot: The `student_groups` state before the change.
    :param new_groups_snapshot: The `student_groups` state after the change.
    :param old_student_group_assignments: A map of student IDs to their original group IDs.
    :param new_student_group_assignments: A map of student IDs to their new group IDs.
    :param old_next_group_id_num: The group ID counter state before management.
    :param new_next_group_id_num: The group ID counter state after management.
    """
...
class ResetSettingsCommand(Command):
    ...
    This command performs a deep-copy snapshot of the entire `app.settings`
    dictionary to ensure a perfect restoration if the reset is undone.
    """
    def __init__(self, app, timestamp=None):
        super().__init__(app, timestamp)
